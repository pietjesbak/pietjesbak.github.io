...
export function Card<T extends React.HTMLAttributes<HTMLDivElement>>(
  props: T & CardProps
) {
  // @ts-ignore Spreading types with generics is not supported (yet).
  const { className, face, value, ...rest } = props;
  const color =
    face === CardFaces.CLUBS || face === CardFaces.SPADES ? "black" : "red";
  let text = "";
    ...
    return (
      <div className="card-deck-table">
        ...
        <div className="revealed-cards">
          {this.state.revealedCards.map((card, i) => (
            <Card key={i} face={card.face} value={card.value} />
          ))}
        </div>
      </div>
    );
