...
class RemotePlayer extends React.Component<
...
> {
  ...
  shouldComponentUpdate(
    newProps: Props & React.HTMLAttributes<HTMLDivElement>
  ) {
    const shouldUpdate =
      this.props.player.cards.length !== this.lastCardCount_ ||
      this.props.player.alive !== this.lastAlive_;
    ...
    if (
      newProps.style &&
      this.props.style &&
      newProps.style.transform !== this.props.style.transform
    ) {
      return true;
    }
  ...
  getCardStyles(index: number) {
    ...
    return {
      transform: `translateX(${(-count / 2 + index) * 10}px) rotate(${
  ...
  renderCards() {
    return this.props.player.cards.map((card, i) => (
      <Card style={this.getCardStyles(i)} key={i} />
    ));
  }
