...
> {
  ...
  renderAnnouncementMessage(announcement: Announcement) {
    return announcement.formattedMessage.map(([subject, text], i) => (
      <span
        key={i}
        className={Announcements.announcementSubjectClasses[subject]}
      >
        {text}
      </span>
    ));
  ...
  renderAnnouncements() {
    ...
    for (let i = announcements.length - 1; i >= 0; i--) {
      ...
      elements.push(
        <li key={"a" + i} className="animated-announcement">
    ...
    elements.push(
      ...announcements.map((announcement, i) => (
        <li key={i}>{this.renderAnnouncementMessage(announcement)}</li>
      ))
    );
  ...
  render() {
    return (
      <ul
        ...
        onTouchStart={this.enterAnnouncements}
        onMouseEnter={this.enterAnnouncements}
      >
