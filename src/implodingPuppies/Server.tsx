...
> {
  ...
  renderButtons() {
    if (location.protocol === "http:") {
      return [
        <button onClick={this.createServer} key="create">
          Create server
        </button>,
        <button onClick={this.joinServer} key="join">
          Join server
        </button>,
      ];
    } else {
      return [
        <button onClick={this.offlineServer} key="offline">
          Play offline
        </button>,
      ];
    }
  ...
  renderForm() {
    ...
    return (
      <div className={classNames("server-dialog", className)} {...rest}>
        ...
        <div>
          <h4>
            ...
            <Tooltip
              ...
              content={
                <div style={{ width: 200 }}>
                  This is your nickname in the game and will be shown to other
                  players.
                </div>
              }
            >
        ...
        <div>
          <h4>
            ...
            <Tooltip
              ...
              content={
                <div style={{ width: 200 }}>
                  Share this key with the people you want to play the game with.
                  Players with the same key join the same server.
                </div>
              }
            >
              <i className="icon-info" />
        ...
        <br />
        <div>
          Imploding puppies is based on{" "}
          <a href="https://explodingkittens.com/">Exploding kittens</a>. You can
          read how to play the game{" "}
          <a href="https://explodingkittens.com/downloads/rules/Exploding_Kittens_Rules.pdf">
            here
          </a>
          .
        </div>
    ...
    return (
      <div
        className={classNames(
          "server-lobby",
          "imploding-puppies-game",
          className
        )}
        {...rest}
      >
        <h3>
          Lobby ({this.state.server!.players.length}/{GameData.MAX_PLAYER_COUNT}
          )
        </h3>
        <div className="server-row">
          <div className="players">
            {this.state.server!.players.map((player) => (
              <div
                className="player player-avatar"
                key={player.id}
          ...
          {this.state.server!.isHost ? (
            <div className="admin-buttons">
              <button
                ...
                disabled={
                  this.state.server!.players.length < GameData.MIN_PLAYER_COUNT
                }
        ...
        <div className="lobby-info">
          {info.decks} deck(s) will be used to play with{" "}
          {this.state.server!.players.length} player(s). There will be{" "}
          {info.deckSize} cards in the deck, including {info.bomb} imploding
          puppies ({cards.get(CardTypes.BOMB)!.icon}) and {info.defuse} defuses
          ({cards.get(CardTypes.DEFUSE)!.icon})!
        </div>
      </div>
    );
  }
