...
> {
  ...
  getCardStyles(i: number, card: CardData, addedCards: CardData[]) {
    ...
    if (addedCards.indexOf(card) !== -1) {
      ...
      transform = `rotate(${Math.sin(i + offset) * 20}deg) translate(${
        Math.sin(i + offset) * 20
      }px, ${Math.cos(i + offset) * 20}px)`;
    }
  ...
  render() {
    const { game, onClick, className, small, getPlayerAngle, ...rest } =
      this.props;
    ...
    return (
      <div
        className={classNames(className, "imploding-puppies-deck-holder", {
          small: small,
        })}
        {...rest}
      >
        {size > 0 ? (
          <div
            onClick={onClick}
            className={classNames("imploding-puppies-deck", "card-deck")}
            style={{
              transform: `translate(${-size * 2}px, ${-size * 2}px)`,
              boxShadow: `${size}px ${size}px ${size}px ${size}px #333`,
            }}
          >
        ...
        <div className={classNames("imploding-puppies-discard")}>
          ...
          {game.discardPile.slice(-10).map((card, i) => (
            <Card
              key={offset + i}
