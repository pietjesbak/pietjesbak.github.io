...
> {
  ...
  componentDidUpdate() {
    const { addedCards, removedCards } = diffCardstate(
      this.state.cards,
  ...
  getCardStyles(index: number, card: CardData, addedCards: CardData[]) {
    ...
    return {
      transform: `translate(${(-count / 2 + index) * 10}px, 50px) rotate(${
        ((-count / 2 + index) * range) / count
      }deg)`,
    };
  }
  ...
  clickCard = (index: number) => (e: React.MouseEvent<HTMLDivElement>) => {
    if (this.state.option === Options.DRAW_PLAY) {
      ...
      if (pos !== -1) {
        ...
        card.prototype.playTest(this.props.player, this.props.player.selection)
      ) {
  ...
  clickButton = (e: React.MouseEvent<HTMLButtonElement>) => {
    if (this.props.player.selection.length === 0) {
      ...
      this.state.callbacks.playCallback!(
        this.props.player.selection.map((card) => card.prototype.type)
      );
    }
  };
  ...
  getPlayButton() {
    ...
    const disabled =
      this.props.player.selection.length !== 0 &&
      !this.props.player.canPlay(this.props.game);

    return (
      <button disabled={disabled} onClick={this.clickButton}>
        {text}
      </button>
    );
  }
  ...
  selectPlayer =
    (player: PlayerData) => (event: React.MouseEvent<HTMLElement>) => {
    ...
    return (
      <div className="card-selection player-overlay">
        <div className="card-container">
          {this.state.cardOptions!.map((type, i) => (
            <Card type={type} onClick={this.selectCard(type)} key={i} />
          ))}
        </div>
      </div>
    );
  ...
  renderFuture() {
    return (
      <div className="future player-overlay">
        <div className="card-container">
          {this.state.future!.map((type, i) => (
            <Card type={type} interactive={false} key={i} />
          ))}
        </div>
  ...
  renderInsertInDeck() {
    return (
      <DeckInsert
        cards={this.state.deckOption!}
        type={CardTypes.BOMB}
        callback={this.state.callbacks.insertCallback!}
      />
    );
  }

  render() {
    const { addedCards } = diffCardstate(
      this.state.cards,
      this.props.player.cards
    );
    ...
    return (
      <div
        className={classNames(
          "imploding-puppies-player",
          { small: this.props.small },
          { active: this.state.option !== Options.NONE }
        )}
      >
        <div
          className={classNames("player", "player-avatar", {
            dead: !this.props.player.alive,
          })}
        ...
        {this.state.option === Options.NOPE ? (
          <button className="nope" onClick={this.state.callbacks.nopeCallback}>
            Nope!
          </button>
        ) : null}
        {this.state.option === Options.SELECT_TARGET
          ? this.renderPlayerSelection()
          : null}
        {this.state.option === Options.SELECT_CARD
          ? this.renderCardSelection()
          : null}
        {this.state.option === Options.SEE_FUTURE ? this.renderFuture() : null}
        {this.state.option === Options.INSERT_IN_DECK
          ? this.renderInsertInDeck()
          : null}
        ...
        <div className="own-cards">
          {this.props.player.cards.map((card, i) => (
            <Card
              ...
              canSelect={
                this.state.option === Options.DRAW_PLAY &&
                card.prototype.playTest(
                  this.props.player,
                  this.props.player.selection
                )
              }
