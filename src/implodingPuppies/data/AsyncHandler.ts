...
export class AsyncHandler {
  ...
  resolve<T extends object>(key: string, data?: Partial<T>) {
    const promise = this.promises_.get(key) as
      | PromiseStore<Partial<T>>
      | undefined;
    if (promise === undefined) {
      throw new Error(
        `Promise ${key} does not exist or has already been resolved!`
      );
    }
  ...
  private get<T = {}>(keys: string[]) {
    return keys
      .map((key) => this.promises_.get(key))
