...
function startServer(
  ...
  server.players.forEach(
    (player) => (player.cards = playerCards!.map((type) => new Card(type)))
  );
...
describe("Server", () => {
  ...
  it("A player can draw a card", async () => {
    ...
    jest
      .spyOn(Player.prototype, "giveOptions")
      .mockImplementation((drawFn: () => void) => {
  ...
  it("A player can play one or more cards", async () => {
    const { server, round } = startServer(3, undefined, [
      CardTypes.ATTACK,
      CardTypes.SKIP,
    ]);
    ...
    jest
      .spyOn(Player.prototype, "giveOptions")
      .mockImplementation(
        (drawFn: () => void, playFn: (selection: CardTypes[]) => void) => {
          if (turn === 0) {
  ...
  it("It should be possible to nope a card", (callback) => {
    ...
    jest
      .spyOn(Player.prototype, "allowNope")
      .mockImplementation((fn: () => void) => nopes.push(fn));
  ...
  it("It should be possible to nope a nope", (callback) => {
    ...
    jest
      .spyOn(Player.prototype, "allowNope")
      .mockImplementation(function (this: Player, fn: () => void) {
        nopes.set(this.id, fn);
      });
  ...
  it("A player plays a defuse when drawing a bomb", async () => {
    const { server, round } = startServer(3, [
      CardTypes.BOMB,
      CardTypes.PUPPY_1,
      CardTypes.PUPPY_2,
    ]);
    ...
    jest
      .spyOn(Player.prototype, "allowInsertIntoDeck")
      .mockImplementation(
        (maxPosition: number, insertCallback: (position: number) => void) => {
  ...
  it("A player plays a favor to gain a card", async () => {
    ...
    jest
      .spyOn(Player.prototype, "allowSelectTarget")
      .mockImplementation(function (
        this: Player,
        options: Player[],
        selectCallback: (player: Player) => void
      ) {
    ...
    jest
      .spyOn(Player.prototype, "allowSelectCard")
      .mockImplementation(function (
        this: Player,
        options: CardTypes[],
        selectCallback: (selection: CardTypes) => void
      ) {
  ...
  it("A player plays a see the future to see the top 3 cards", async () => {
    const { server, round } = startServer(
      3,
      [CardTypes.BOMB, CardTypes.DEFUSE, CardTypes.ATTACK, CardTypes.PUPPY_1],
      [CardTypes.FUTURE]
    );
    ...
    jest
      .spyOn(Player.prototype, "seeFuture")
      .mockImplementation(function (
        this: Player,
        cards: CardTypes[],
        confirmFn: () => void
  ...
  it("A player plays a shuffle to shuffle the deck", async () => {
    const { server, round } = startServer(
      3,
      [CardTypes.BOMB, CardTypes.DEFUSE, CardTypes.ATTACK],
      [CardTypes.SHUFFLE]
    );
  ...
  it("A player plays 2 puppies to steal a card", async () => {
    const { players, server, round } = startServer(3, undefined, [
      CardTypes.PUPPY_1,
      CardTypes.PUPPY_1,
    ]);
    ...
    jest
      ...
      .mockImplementation(function (
        ...
        first
          ? playCallback([CardTypes.PUPPY_1, CardTypes.PUPPY_1])
          : server.shutDown();
        first = false;
      });
    jest
  ...
  it("A player plays 5 different cards to take a card from the discard pile", async () => {
    const { players, server, round } = startServer(
      3,
      undefined,
      [
        CardTypes.PUPPY_1,
      ...
      [CardTypes.DEFUSE, CardTypes.SHUFFLE]
    );
  ...
  it("A player nopes another player trying to steal from him", async () => {
    ...
    jest
      .spyOn(Player.prototype, "allowNope")
      .mockImplementation(function (this: Player, nopeCallback: () => void) {
  ...
  it("It is not possible to play incorrect combinations", async () => {
    ...
    jest
      ...
      .mockImplementation(function (
        ...
        first
          ? playCallback([CardTypes.PUPPY_1, CardTypes.PUPPY_2])
          : server.shutDown();
        first = false;
      });
