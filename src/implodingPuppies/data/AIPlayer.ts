...
export class AIPlayer extends BotConnection {
    ...
    send(data: PeerData) {
        switch (data.type) {
            case DataType.JOIN:
                ...
                this.playerKarma_ = new Map((data.players as Array<{ id: number }>).map(({ id }) => [id, Math.random() - 0.5] as [number, number]));
            ...
            case DataType.ANNOUNCEMENT:
                this.processAnnouncement(Announcement.deserialize(this.game_, data.announcement));
                break;
            ...
            case DataType.GIVE_OPTIONS:
                ...
                const lastBombedKarma = this.playerKarma_.get(this.lastBombedPlayer_) || 0;
                const carefulness = this.lastBombDeckSize_ === -1 ? 0 : (1 / ((this.lastBombDeckSize_ - this.deckSize_) || 1)) * 0.5 + clamp(-2, lastBombedKarma, 2) * -0.2;
            ...
            case DataType.PLAYER_SELECT:
                ...
                const playerArr = [...this.playerKarma_.entries()].sort((a, b) => a[1] - b[1]);

                this.dataAction({
                    type: DataType.PLAYER_SELECT,
                    player: playerArr.find(([id, karma]) => options.indexOf(id) !== -1)![0]
                });
                break;

            case DataType.INSERT_CARD:
                const maxPosition = data.position as number;
                const target = [...this.playerKarma_.entries()].sort((a, b) => a[1] - b[1])[0][0];

                let newPosition = 0;
                let playerIndex = (this.currentPlayer_ + 1) % this.game_.playerCount;
                while (newPosition < maxPosition) {
                    if (playerIndex === target && Math.random() > 0.7 - (newPosition / 10)) {
                        break;
                    }
                    newPosition++;
                }
            ...
            case DataType.NOPE:
                ...
                if (this.hasCards([CardTypes.NOPE]) &&
                    (this.currentTarget_ === this.ownId_ && this.currentNopeResult_ === false && AIPlayer.actionThreats.get(this.currentAction_)! > actionThreshold) ||
                    (this.currentPlayer_ === this.ownId_ && this.currentNopeResult_ === true && AIPlayer.actionThreats.get(this.currentAction_)! > actionThreshold) ||
                    (this.currentTarget_ === -1 && this.currentNopeResult_ === false && AIPlayer.actionThreats.get(this.currentAction_)! > actionThreshold)) {
    ...
    processAnnouncement(announcement: Announcement) {
        ...
        switch (announcement.type) {
            ...
            case AnnouncementTypes.PUT_BOMB:
                ...
                if (this.lastBombedPlayer_ !== this.ownId_ && this.lastBombedPlayer_ !== -1 && this.currentPlayer_ === this.ownId_) {
                    this.playerKarma_.set(this.lastBombedPlayer_, this.playerKarma_.get(this.lastBombedPlayer_)! - 3);
                }
    ...
    processActionConsequences() {
        ...
        if (this.currentPlayer_ !== this.ownId_) {
            if (this.currentTarget_ === this.ownId_) {
                this.playerKarma_.set(this.currentPlayer_, this.playerKarma_.get(this.currentPlayer_)! - consequence * Math.random() / 2 + 0.5);
            } else {
                this.playerKarma_.set(this.currentPlayer_, this.playerKarma_.get(this.currentPlayer_)! + consequence * Math.random() / 2);
            }
        }
    }
    ...
    tryPlay() {
        ...
        if (this.hasCards([CardTypes.SHUFFLE]) && !this.checkIfPlayedThisRound(CardTypes.SHUFFLE)) {
            options.push({
                selection: [CardTypes.SHUFFLE],
                weight: (1 - this.getModifiedBombProbability()) * (Math.random() / 2 + 0.5)
            });
        }

        if (this.hasCards([CardTypes.FUTURE]) &&
            !this.checkIfPlayedThisRound(CardTypes.FUTURE) &&
            this.bombPositions_[this.bombPositions_.length - 1] > this.game_.deck.cards.length) {
        ...
        if (this.hasCards([CardTypes.SKIP])) {
            options.push({
                ...
                weight: this.bombPositions_[this.bombPositions_.length - 1] === this.game_.deck.cards.length ? 0.8 : 0.1
            });
        }
        ...
        if (this.hasCards([CardTypes.ATTACK])) {
            options.push({
                ...
                weight: this.bombPositions_[this.bombPositions_.length - 1] === this.game_.deck.cards.length ? 0.6 : 0.1
            });
        }
        ...
        [CardTypes.PUPPY_1, CardTypes.PUPPY_2, CardTypes.PUPPY_3, CardTypes.PUPPY_4, CardTypes.PUPPY_5].forEach((type) => this.addOfAKindOption(type, 0.3, options));
        [CardTypes.NOPE, CardTypes.FAVOR, CardTypes.SHUFFLE].forEach((type) => this.addOfAKindOption(type, 0.2, options));

        if (this.discardPile_.find(type => type === CardTypes.DEFUSE) !== undefined &&
            new Set(this.ownCards_.filter(type => type !== CardTypes.DEFUSE)).size >= 5) {

            const scores = this.getCardsScore(this.ownCards_).sort((a, b) => b.score - a.score);
