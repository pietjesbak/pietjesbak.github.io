...
const getApplicableCount = (types: Card[]) => {
  return new Set(
    types.filter((card) => card.prototype.type !== CardTypes.DEFUSE)
  ).size;
};
...
const fiveDifferent = (player: Player, selection: Card[], type: CardTypes) => {
  return (
    getApplicableCount(player.cards) >= 5 &&
    selection.length < 5 &&
...
const twoOrThreeSame = (player: Player, selection: Card[], type: CardTypes) => {
  return (
    player.cards.filter((card) => card.prototype.type === type).length >= 2 &&
    selection.length < 3 &&
...
cards.set(CardTypes.SHUFFLE, {
  ...
  count: 4,
  requireTarget: false,
  playTest: (player, selection) =>
    selection.length === 0 ||
    fiveDifferent(player, selection, CardTypes.SHUFFLE) ||
    twoOrThreeSame(player, selection, CardTypes.SHUFFLE),
  playEffect: async (player, game) => {
    game.announce(
      new Announcement(AnnouncementTypes.SHUFFLE, undefined, player)
    );
    game.deck.shuffle();
  },
  score: (hand: CardTypes[], game: Game) =>
    Math.floor(game.deck.cards.length / 10) + count(hand, CardTypes.SHUFFLE),
});
...
cards.set(CardTypes.NOPE, {
  ...
  requireTarget: false,
  playTest: (player, selection) =>
    fiveDifferent(player, selection, CardTypes.NOPE) ||
    twoOrThreeSame(player, selection, CardTypes.NOPE),
...
cards.set(CardTypes.SKIP, {
  ...
  playTest: (player, selection) =>
    selection.length === 0 ||
    fiveDifferent(player, selection, CardTypes.SKIP) ||
    twoOrThreeSame(player, selection, CardTypes.SKIP),
...
cards.set(CardTypes.ATTACK, {
  ...
  name: "Attack",
  description:
    "Use this card to skip drawing a card and make the next player take 2 turns",
  count: 4,
  requireTarget: false,
  playTest: (player, selection) =>
    selection.length === 0 ||
    fiveDifferent(player, selection, CardTypes.ATTACK) ||
    twoOrThreeSame(player, selection, CardTypes.ATTACK),
...
cards.set(CardTypes.FAVOR, {
  ...
  count: 4,
  requireTarget: true,
  announce: (player, game, target) =>
    new Announcement(AnnouncementTypes.FAVOR_FROM, undefined, player, target),
  playTest: (player, selection) =>
    selection.length === 0 ||
    fiveDifferent(player, selection, CardTypes.FAVOR) ||
    twoOrThreeSame(player, selection, CardTypes.FAVOR),
  playEffect: async (player, game, target) => {
    if (target!.cards.length > 0) {
      const options = [
        ...new Set(target!.cards.map((card) => card.prototype.type)),
      ];
      const selectedType = (await game.selectCard(target!, options)).data!
        .selection;
...
cards.set(CardTypes.FUTURE, {
  ...
  playTest: (player, selection) =>
    selection.length === 0 ||
    fiveDifferent(player, selection, CardTypes.FUTURE) ||
    twoOrThreeSame(player, selection, CardTypes.FUTURE),
...
[
  ...
  cards.set(type, {
    ...
    name: "Animal",
    description:
      "A collectable. Combine 2 or 3 of the same type to steal a card.",
    count: 4,
    requireTarget: false,
    playTest: (player, selection) =>
      fiveDifferent(player, selection, type) ||
      twoOrThreeSame(player, selection, type),
