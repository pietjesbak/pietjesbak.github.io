...
export class Game extends AsyncHandler {
  ...
  forceStart() {
    if (
      this.playerCount_ >= Game.MIN_PLAYER_COUNT &&
      this.startKey_ !== undefined
    ) {
  ...
  initDeck() {
    ...
    Object.values(CardTypes)
      .filter(
        (type: CardTypes) =>
          type !== CardTypes.BOMB && type !== CardTypes.DEFUSE
      )
  ...
  async waitForPlayers(): Promise<Player[]> {
    this.slots_ = new Array(Game.MAX_PLAYER_COUNT)
      .fill(null)
      .map(() => this.createPromise(AsyncActions.JOIN));
    ...
    await Promise.race([
      ...
      this.getPromise(this.startKey_!).catch((reject) => reject),
    ]);
    ...
    let data = (await Promise.all(
      promises.map((promise) => promise.catch((reject) => reject))
    )) as Array<AsyncData<AsyncJoin>>;
    return data
  ...
  async gameLoop() {
    ...
    while (!gameOver) {
      this.clearPlayers_();
      const keys = [
        this.createPromise(AsyncActions.DRAW),
        this.createPromise(AsyncActions.PLAY),
      ];
      const promises = this.getPromises(keys);

      this.currentPlayer.giveOptions(
        this.createDefaultAction(keys[0]),
        this.createPlayAction(keys[1])
      );
      this.update_();
      ...
      try {
        result = await Promise.race([
          ...promises,
          this.getPromise<AsyncPlay>(this.runningKey_),
        ]);
      } catch (e) {
        // Server shut down.
      ...
      switch (result.action) {
        ...
        case AsyncActions.PLAY:
          await this.playerPlay(
            (result as AsyncData<AsyncPlay>).data!.selection
          );
          break;
      }
  ...
  async playerDraw() {
    ...
    this.announce(
      new Announcement(
        AnnouncementTypes.DRAW_CARD,
        card.prototype.type === CardTypes.BOMB ? [CardTypes.BOMB] : undefined,
        this.currentPlayer
      )
    );
  ...
  async playerPlay(selection: CardTypes[]) {
    ...
    if (play !== undefined) {
      ...
      if (play.requireTarget(selection, this.currentPlayer, this)) {
        this.announce(
          new Announcement(
            AnnouncementTypes.SELECT_TARGET,
            selection,
            this.currentPlayer
          )
        );
  ...
  selectTarget() {
    this.clearPlayers_();
    const options = this.players_.filter(
      (player) => player.alive && player !== this.currentPlayer
    );
    const key = this.createPromise<AsyncSelectPlayer>(
      AsyncActions.SELECT_PLAYER,
      {
        source: this.currentPlayer,
        options,
      }
    );
    this.currentPlayer.allowSelectTarget(
      options,
      this.createSelectPlayerAction(key)
    );
  ...
  async insertIntoDeck(type: CardTypes) {
    ...
    this.currentPlayer.allowInsertIntoDeck(
      this.deck.cards.length,
      this.createInsertCardAction(key)
    );

    const position = (await this.getPromise<AsyncInsertCard>(key)).data!
      .position;
    this.announce(
      new Announcement(
        AnnouncementTypes.PUT_BOMB,
        undefined,
        this.currentPlayer
      )
    );
  ...
  async processNopes(selection: CardTypes[]) {
    ...
    do {
      this.clearPlayers_();
      const keys = repeat(this.playerCount_).map((unused, i) =>
        this.createPromise(
          AsyncActions.NOPE,
          { player: this.players_[i] },
          Game.NOPE_TIMEOUT_MILLIS
        )
      );
      ...
      if (player !== undefined) {
        ...
        this.announce(
          new Announcement(
            result ? AnnouncementTypes.NOPE : AnnouncementTypes.YUP,
            selection,
            excludedPlayer,
            target
          )
  ...
  async processFuture() {
    ...
    this.announce(
      new Announcement(AnnouncementTypes.FUTURE, undefined, this.currentPlayer)
    );
  ...
  private getNextAlivePlayer_() {
    ...
    do {
      currentPlayer =
        this.players[++this.currentPlayerIndex_ % this.players.length];
