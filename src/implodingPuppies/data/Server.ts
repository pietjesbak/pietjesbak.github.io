...
export class Server extends PeerBase {
  ...
  start = () => {
    if (this.players.length >= 2) {
      ...
      this.ownId_ = this.connections_.find(
        (conn) => conn.connection === undefined
      )!.player.id;
      this.started_ = true;
      this.update_();
  ...
  updateState = () => {
    const base = {
      ...
      discard: this.game.discardPile.map((card) => ({
        type: card.prototype.type,
        owner: card.owner.data,
      })),
          ...
          return {
            ...
            cards:
              connection.player === player
                ? player.cards.map((card) => card.prototype.type)
                : repeat(player.cards.length).map(() => CardTypes.BOMB),
          };
        }),
      };
    });
      ...
      try {
        switch (data.type) {
          ...
          case DataType.DRAW:
            if (
              conn !== undefined &&
              conn.callbacks.drawCallback !== undefined
            ) {
          ...
          case DataType.PLAY:
            if (
              conn !== undefined &&
              conn.callbacks.playCallback !== undefined
            ) {
          ...
          case DataType.NOPE:
            if (
              conn !== undefined &&
              conn.callbacks.nopeCallback !== undefined
            ) {
          ...
          case DataType.PLAYER_SELECT:
            if (
              conn !== undefined &&
              conn.callbacks.playerSelectCallback !== undefined
            ) {
              conn.callbacks.playerSelectCallback(
          ...
          case DataType.CARD_SELECT:
            if (
              conn !== undefined &&
              conn.callbacks.cardSelectCallback !== undefined
            ) {
          ...
          case DataType.INSERT_CARD:
            if (
              conn !== undefined &&
              conn.callbacks.insertCallback !== undefined
            ) {
          ...
          case DataType.CONFIRM:
            if (
              conn !== undefined &&
              conn.callbacks.confirmCallback !== undefined
            ) {
        ...
        console.log(
          `Disconnecting player ${
            conn && conn.player.name
          } due to an error while processing their command`
        );
        console.error(e);
        connection.close();
      }
