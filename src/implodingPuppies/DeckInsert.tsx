...
class DeckInsert extends React.PureComponent<
...
> {
  get index() {
    return clamp(
      0,
      Math.round(((this.state.x - 35) / this.state.width) * this.props.cards),
      this.props.cards
    );
  }
  ...
  renderCards(amount: number, offset: number) {
    return repeat(amount).map((unused, i) => (
      <Card
        key={i + offset}
        interactive={false}
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Card
          type={type}
          style={{ transform: `translate(${this.state.x - 50}px, 20px)` }}
        />
        {this.renderCards(cards - index, index)}
        <span className="count left-count">
          {index}
          <span>Bottom</span>
        </span>
        <span className="count right-count">
          {this.props.cards - index}
          <span>Top</span>
        </span>
      </div>
    );
