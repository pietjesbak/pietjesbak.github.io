...
class BggGameData {
    ...
    constructor(node: Element) {
        ...
        this.name = String(node.getElementsByTagName('name')[0].childNodes[0].nodeValue);
        this.year = undefined;
        this.image = String(node.getElementsByTagName('image')[0].childNodes[0].nodeValue);
        this.thumbnail = String(node.getElementsByTagName('thumbnail')[0].childNodes[0].nodeValue);
...
class BggDetailsData {
    ...
    constructor(node: Element, games: Map<number, BggGameData>) {
        this.description = String(node.getElementsByTagName('description')[0].childNodes[0].nodeValue);
        this.categories = [...node.getElementsByTagName('boardgamecategory') as any].filter(filter).map((element: Element) => String(element.childNodes[0].nodeValue));
        ...
        this.mechanics = [...node.getElementsByTagName('boardgamemechanic') as any].filter(filter).map((element: Element) => String(element.childNodes[0].nodeValue));

        this.expansions = new Map();
        [...node.getElementsByTagName('boardgameexpansion') as any]
            .filter(filter)
            .forEach((element: Element) => this.expansions.set(Number(element.attributes['objectid'].nodeValue), String(element.childNodes[0].nodeValue)));


        this.domain = [...node.getElementsByTagName('boardgamesubdomain') as any].filter(filter).map((element: Element) => String(element.childNodes[0].nodeValue));
